...
class MaskKeypointRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_keypoint:
            keypoint_results = self._keypoint_forward_train(x, sampling_results,
                                                            bbox_results['bbox_feats'],
                                                            gt_keypoints, img_metas)
    ...
    def _keypoint_forward_train(self, x, sampling_results, bbox_feats, gt_keypoints,
                            img_metas):
        if not self.share_roi_extractor:
            pos_rois = bbox2roi([res.pos_bboxes for res in sampling_results])
            if pos_rois.shape[0] == 0:
                return dict(loss_mask=None)
            keypoint_results = self._keypoint_forward(x, pos_rois)
        else:
            pos_inds = []
            device = bbox_feats.device
            for res in sampling_results:
        ...
        keypoint_targets, valids = self.keypoint_head.get_target(sampling_results, gt_keypoints,
                                              self.train_cfg)
        loss_keypoint = self.keypoint_head.loss(keypoint_results['keypoint_pred'],
                                                keypoint_targets, valids)
        keypoint_results.update(loss_keypoint=loss_keypoint, keypoint_targets=keypoint_targets)
